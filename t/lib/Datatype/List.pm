...
sub t001_initialize_hessian : Test(2) {    #{{{
    ...
    ok(
        $hessian_obj->can('deserialize_chunk'),
...
sub t010_read_fixed_length_typed : Test(1) {    #{{{
    ...
    my $datastructure =
      $hessian_obj->deserialize_chunk( { input_string => $hessian_data } );
...
sub t011_read_variable_length_typed : Test(1) {    #{{{
    ...
    my $datastructure =
      $self->{deserializer}->deserialize_chunk( { input_string => $hessian_data } );
    cmp_deeply(
        $datastructure,
        [ 0, 1, 262143 ],
...
sub class_instance_generator {    #{{{
    ...
    my $simple_obj =
      $self->{deserializer}->deserialize_chunk( { input_handle => $ih } );
